...
homekit_service_t *create_relay_service(int id)
{
    int relay_name_len = snprintf(NULL, 0, "OUTLET %d", id + 1);
    char *relay_name_value = malloc(relay_name_len + 1);
    snprintf(relay_name_value, relay_name_len + 1, "OUTLET %d", id + 1);
    return NEW_HOMEKIT_SERVICE(
        OUTLET,
        .characteristics = (homekit_characteristic_t *[]){
            NEW_HOMEKIT_CHARACTERISTIC(
                NAME, relay_name_value),
            NEW_HOMEKIT_CHARACTERISTIC(
                ON, false,
                .callback = HOMEKIT_CHARACTERISTIC_CALLBACK(
                    relay_callback, .context = (void *)&relay_gpios[id]), ),
            NULL});
}
...
void init_accessory()
{
    ...
    *(s++) = NEW_HOMEKIT_SERVICE(
        ...
        .characteristics = (homekit_characteristic_t *[]){
            ...
            NEW_HOMEKIT_CHARACTERISTIC(MODEL, "Switch"),
